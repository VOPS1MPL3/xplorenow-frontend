...
let scale = 1; function applyScale(){ svgContainer.style.transform = 'scale(' + scale + ')'; svgContainer.style.transformOrigin = '50% 50%'; }
...
async function loadFloor(f){
    ...
    try{
        ...
        // --- inline of previous processSVG(svg, f) ---
        ...
        loadPinnedState();
        // Call applyFilter but disable auto-switch to avoid recursion when loadFloor triggers it
        applyFilter('', { allowSwitch: false });
        // --- end inline ---
    }catch(e){
        console.error('Failed to load svg', e);
        // Provide a helpful message for the user. When the page is opened via file:// many browsers
        // block fetch() from loading local files. In that case instruct the user to run a tiny
        // local server (recommended) or open the site via HTTP.
        svgContainer.innerHTML = `
            <div class="text-center text-muted">
                <div style="font-weight:600;margin-bottom:8px">No se pudo cargar el plano</div>
                <div>Esto suele ocurrir si abriste el proyecto con <code>file://</code>. Para que los planos SVG se carguen correctamente, abre el proyecto desde un servidor local.</div>
                <div style="margin-top:8px; text-align:left; display:inline-block; background:#f7f7f7; padding:8px; border-radius:6px;">
                    <div style="font-weight:600;margin-bottom:6px">Comandos recomendados (PowerShell)</div>
                    <div><code>python -m http.server 8000</code></div>
                    <div style="margin-top:6px"><code>npx http-server -p 8000</code> (si tienes Node)</div>
                </div>
                <div style="margin-top:10px">Luego abre <a href="http://localhost:8000" target="_blank">http://localhost:8000</a> en tu navegador y navega a la carpeta del proyecto.</div>
            </div>`;
    }
}

function rebuildCards(filteredKeys){
    // categories (derived from entries on the current floor)
    const cats = new Set(Object.keys(storeMap).filter(k=> storeMap[k].floor === currentFloor.val).map(k=>storeMap[k].category || 'Otros'));
    ...
    // Determine which keys to render: use provided filteredKeys (array of keys) or default to all keys on current floor
    let keysToRender = [];
    if(Array.isArray(filteredKeys)){
        keysToRender = filteredKeys.slice();
    }else{
        keysToRender = Object.keys(storeMap).filter(k=> storeMap[k].floor === currentFloor.val);
    }
    keysToRender.sort((a,b)=>Number(a)-Number(b)).forEach(idx=>{
...
async function applyFilter(cat='', options = { allowSwitch: true }){
    ...
    Object.keys(storeMap).forEach(k=>{
        ...
        // pins should only be shown if pinned, on current floor and match filters
        const showPin = s.pinned && s.floor===currentFloor.val && matchesCat && matchesQ;
        if(s.pin){ s.pin.style.display = showPin ? 'inline' : 'none'; if(!s.pinned) s.pin.classList.add('hidden'); }

        // cards: include entries that match category + query and belong to current floor (independent of pinned state)
        if(matchesCat && matchesQ && s.floor === currentFloor.val){ filtered.push(k); }
    });

    // If no matches found on the current floor, but the query matches known names/codes on another floor,
    // auto-switch to that floor (only if allowed). We use the static mapping tables (displayNameMappings/codeMappings)
    // to detect possible matches on floors that aren't loaded yet.
    if(filtered.length === 0 && q && allowSwitch){
        const matchesByFloor = {};
        Object.keys(displayNameMappings).forEach(floorKey => {
            const map = displayNameMappings[floorKey] || {};
            Object.keys(map).forEach(idx => {
                const name = (map[idx]||'').toLowerCase();
                const code = ((codeMappings[floorKey] && codeMappings[floorKey][idx])||'').toLowerCase();
                if(name.includes(q) || code.includes(q)){
                    matchesByFloor[floorKey] = matchesByFloor[floorKey] || [];
                    matchesByFloor[floorKey].push(idx);
                }
            });
        });

        // prefer a different floor than current
        const floorsWithMatches = Object.keys(matchesByFloor).filter(f => f !== currentFloor.val);
        if(floorsWithMatches.length > 0){
            const targetFloor = floorsWithMatches[0];
            await loadFloor(targetFloor);
            // loadFloor will call applyFilter again with allowSwitch:false, so we can return now
            return;
        }
    }
...
// Floor tab handling
document.querySelectorAll('.floor-tabs label').forEach(l=> l.addEventListener('click', ()=>{ const f = l.getAttribute('data-floor'); loadFloor(f); }));

// search input
storeSearch.addEventListener('input', ()=> applyFilter());

// initial load
(async()=>{ await loadFloor('1'); })();
